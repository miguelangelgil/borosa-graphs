...
Fetches gold futures prices and real options data from Yahoo Finance and COT data from CFTC
"""
...
# Gold futures symbol for price data
GOLD_FUTURES_SYMBOL = "GC=F"

# Gold ETF symbol for options data (GLD has liquid options market)
GOLD_ETF_SYMBOL = "GLD"

# CFTC COT Report API
# Gold futures contract code: 088691 (Gold - Commodity Exchange Inc.)
...
def fetch_cot_data():
    """
    Fetch Commitment of Traders (COT) data from CFTC for gold
    Returns weekly positioning data by trader type
    """
    print("Fetching COT Report data from CFTC...")
    ...
    try:
        ...
        formatted_data = []
        for record in reversed(cot_data):  # Oldest first
            try:
...
def fetch_gold_prices():
    """Fetch historical gold futures prices"""
    print("Fetching gold futures prices from Yahoo Finance...")

    try:
        # Fetch 6 months of data
        gold = yf.Ticker(GOLD_FUTURES_SYMBOL)
...
def fetch_real_options_data():
    """
    Fetch real options data from Yahoo Finance
    Returns options chain data for GLD (SPDR Gold Shares ETF)
    """
    print(f"Fetching real options data for {GOLD_ETF_SYMBOL} from Yahoo Finance...")

    try:
        gold = yf.Ticker(GOLD_ETF_SYMBOL)

        # Get available expiration dates
        expirations = gold.options
        ...
        # Fetch options for multiple expiration dates (up to 12 for better temporal visualization)
        for expiration in expirations[:12]:
            try:
                print(f"Fetching options for expiration: {expiration}")
                opt_chain = gold.option_chain(expiration)
...
def generate_heatmap_from_options(options_data, current_price):
    ...
    for option in options_data:
        ...
        if call_volume > 0 or put_volume > 0 or call_oi > 0 or put_oi > 0:
            heatmap_data.append({
                "date": expiration,  # Use 'date' for frontend compatibility (represents expiration date)
                "strike": int(strike),
    ...
    # Remove duplicates and sort by date (expiration) and strike
    seen = set()
    unique_data = []
...
def save_snapshot_to_history(current_data):
    """
    ...
    snapshot = {
        ...
        "gold_futures_price": current_data["metadata"]["gold_futures_price"],
        "gld_etf_price": current_data["metadata"]["gld_etf_price"],
        "options_summary": {
        ...
        "top_strikes": {
            "calls": sorted(
                ...
                reverse=True
            )[:5],  # Top 5 call strikes by OI
            "puts": sorted(
                current_data["price_levels"],
                key=lambda x: x["total_put_oi"],
                reverse=True
            )[:5]   # Top 5 put strikes by OI
        },
...
def main():
    try:
        # Fetch gold futures prices
        gold_futures_price, price_history = fetch_gold_prices()

        if gold_futures_price is None or price_history is None:
            print("Failed to fetch gold price data")
            return

        # Get GLD ETF current price (for options context)
        print(f"Fetching {GOLD_ETF_SYMBOL} current price...")
        gld_ticker = yf.Ticker(GOLD_ETF_SYMBOL)
        gld_hist = gld_ticker.history(period="1d")
        gld_current_price = gld_hist['Close'].iloc[-1] if not gld_hist.empty else None

        if gld_current_price is None:
            print(f"Warning: Could not fetch {GOLD_ETF_SYMBOL} current price")
            gld_current_price = gold_futures_price / 10  # Rough approximation

        print(f"Gold Futures (GC=F): ${gold_futures_price:.2f}/oz")
        print(f"GLD ETF Price: ${gld_current_price:.2f}")

        # Fetch real options data
        options_data = fetch_real_options_data()
        ...
        if not options_data:
            ...
            # Generate heat map from real options data (use GLD price for options context)
            heatmap_data = generate_heatmap_from_options(options_data, gld_current_price)
            price_levels = calculate_price_levels(heatmap_data)
            source_note = f"Real {GOLD_ETF_SYMBOL} options data from Yahoo Finance ({len(options_data)} contracts)"
        ...
        result = {
            "metadata": {
                "source": f"Yahoo Finance (GC=F futures & {GOLD_ETF_SYMBOL} options), CFTC COT Report (positioning)",
                "indicator": "Gold Futures & Options Positioning",
                "description": f"Real gold futures prices (GC=F) and {GOLD_ETF_SYMBOL} ETF options chain data. Note: {GOLD_ETF_SYMBOL} represents ~1/10 oz of gold.",
                "fetched_at": datetime.now().isoformat(),
                "gold_futures_price": round(gold_futures_price, 2),
                "gld_etf_price": round(gld_current_price, 2),
                "current_price": round(gold_futures_price, 2),  # Keep for backward compatibility
        ...
        save_snapshot_to_history(result)
        print(f"Gold Futures Price (GC=F): ${gold_futures_price:.2f}/oz")
        print(f"GLD ETF Price: ${gld_current_price:.2f}/share")
        ...
        if options_data:
            ...
            print(f"  Put/Call Volume Ratio: {total_put_volume/total_call_volume:.2f}" if total_call_volume > 0 else "  Put/Call Volume Ratio: N/A")
